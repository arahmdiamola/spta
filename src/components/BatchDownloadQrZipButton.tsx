...
export default function BatchDownloadQrZipButton({ parentsCount }: { parentsCount: number }) {
  const [downloading, setDownloading] = useState(false);
  const [progress, setProgress] = useState(0);

  const handleBatchDownload = async () => {
    const cards = document.querySelectorAll<HTMLDivElement>("[data-qr-card-export]");
    ...
    try {
      ...
      for (let i = 0; i < cards.length; i++) {
        ...
        zip.file(`${parentName.replace(/\s+/g, '_')}_QR_Pass.png`, blob);
        
        setProgress(Math.round(((i + 1) / cards.length) * 100));
      }

      const zipBlob = await zip.generateAsync({ type: "blob" });
      saveAs(zipBlob, "SPTA_QR_Passes.zip");

    } catch (error) {
      console.error("Failed to batch download QR passes", error);
      alert("Failed to batch download QR passes. Please try again.");
    } finally {
